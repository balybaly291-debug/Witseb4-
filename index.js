...
async function startBot() {
    ...
    const { state, saveCreds } = await useRedisAuthState(redis, 'wa-session');
    ...
    if (!sock.authState.creds.registered && !fs.existsSync('./auth_info_baileys/creds.json')) {
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        // تجاهل روابط فيسبوك وتيك توك وانستقرام
        const isSocialLink = /(facebook\.com|fb\.com|fb\.watch|tiktok\.com|vm\.tiktok\.com|instagram\.com|instagr\.am)/i.test(content);
        if (isSocialLink) {
            console.log(`🔕 تجاهل رابط سوشيال من: ${sender}`);
            return;
        }
    ...
    // ✅ حذف الرسالة عند اللايك (فقط الرسائل النصية)
    sock.ev.on('messages.reaction', async (reactions) => {
        for (const reaction of reactions) {
            ...
            // جلب الرسالة الأصلية للتحقق من نوعها
            try {
                const store = await sock.loadMessage(from, reaction.key.id);
                if (!store) {
                    console.log(`⚠️ ما قدرنا نجلب الرسالة`);
                    continue;
                }
                const msgType = Object.keys(store.message || {})[0];
                const isTextMsg = msgType === 'conversation' || msgType === 'extendedTextMessage';
                if (!isTextMsg) {
                    console.log(`🔕 تجاهل لايك على ${msgType}`);
                    continue;
                }
                console.log(`👆 لايك على كتابة - جاري الحذف`);
                await sock.sendMessage(from, { delete: reaction.key });
                console.log(`🗑️ تم الحذف عبر اللايك`);
            } catch (err) {
